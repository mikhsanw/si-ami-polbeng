<div class="accordion-item">
    <h2 class="accordion-header" id="header_{{ $item->id }}">
        <button
            class="accordion-button fs-4 fw-semibold @if (!$loop->first && $loop->count > 1) collapsed @endif"
            type="button" data-bs-toggle="collapse" data-bs-target="#body_{{ $item->id }}"
            aria-expanded="{{ $loop->first ? 'true' : 'false' }}"
            aria-controls="body_{{ $item->id }}">
    ...
    @if ($item->indikators->isEmpty())
        ...
        <button class="btn btn-action btn-sm btn-light-success" 
            data-title="Isi Indikator"
            data-action="create-child"
            data-id="{{ $item->id }}"
            data-url="{{ config('master.app.url.backend') . '/' . $page->url . '/create-indikator/' . $item->id }}">
    ...
    <ul class="list-group list-group-flush">
            ...
            <li class="list-group-item d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center gap-2">
                    <a href="#" class="btn-action" 
                        ...
                        data-id="{{ $item->id }}"
                        data-url="{{ config('master.app.url.backend') . '/' . $page->url . '/edit-indikator/' . $indikator->id }}">
                        {{ $indikator->nama }}
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <button class="btn btn-action btn-sm btn-light-danger" 
                        ...
                        data-action="delete-indikator"
                        data-id="{{ $indikator->id }}"
                        data-url="{{ config('master.app.url.backend') . '/' . $page->url . '/delete-indikator/' . $indikator->id }}">
        ...
        <button class="btn btn-action btn-sm btn-light-success mt-3" 
            data-title="Isi Indikator"
            data-action="create-indikator"
            data-id="{{ $item->id }}"
            data-url="{{ config('master.app.url.backend') . '/' . $page->url . '/create-indikator/' . $item->id }}">
            <i class="ki-outline ki-plus-square fs-3"></i>
            Tambah Indikator
    ...
    @endif
            ...
            <div class="mt-4 pt-4 border-top">
                @can($page->code . ' create')
                    <button class="btn btn-action btn-sm btn-light-primary" 
                        data-title="Tambah"
                        data-action="create-child"
                        data-id="{{ $item->id }}"
                        data-url="{{ config('master.app.url.backend') . '/' . $page->url . '/create-child/' . $item->id }}">
                        <i class="ki-outline ki-plus-square fs-3"></i>
                        Tambah Sub-Kriteria
                    </button>
                @endcan

                <button class="btn btn-sm btn-light-danger btn-action"
                        data-id="{{ $item->id }}"
                ...
                <button class="btn btn-sm btn-light-warning btn-action"
                        data-id="{{ $item->id }}"
                        data-url="{{ config('master.app.url.backend') . '/' . $page->url }}"
