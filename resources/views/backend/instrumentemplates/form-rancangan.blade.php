    ...
    <div class="panel-body">
        <div class="row">
            <div class="col-md-8">
                ...
                <p class="text-muted">Lembaga Akreditasi: <strong>{{ $data->LembagaAkreditasi->nama ?? 'Belum ditentukan' }}</strong></p>
                <hr>
                ...
                @if(isset($data) && $data->lembaga_akreditasi_id)
                    ...
                    <div id="indikator-tree-view" class="tree-view">
                        ...
                        <p class="text-muted text-center"><i class="fas fa-spinner fa-spin me-2"></i>Memuat indikator...</p>
                    </div>
                @else
                    ...
                    @if(isset($data) && $data->templateIndikators->isNotEmpty())
                        @foreach($data->templateIndikators as $templateIndikator)
                            <div class="mb-3 input-bobot" id="bobot-{{ $templateIndikator->indikator_id }}">
                                ...
                                <input type="number" step="0.01" name="bobot[{{ $templateIndikator->indikator_id }}]" class="form-control" placeholder="Contoh: 1,02 (gunakan koma)" value="{{ $templateIndikator->bobot ?? '' }}">
                            </div>
                        @endforeach
                    @endif
                </div>
...
<style>
    /* Styling Anda */
    .tree-view .list-group-item { border: none; cursor: pointer; }
    .tree-view .child { padding-left: 25px; }
    .list-group-item { padding: 5px 0px; margin-top: 5px }
    .list-group-item .form-check { margin-right: 0px; padding-left: 0px; }
    .list-group-item i { width: 20px; text-align: center; } /* Memastikan ikon sejajar */
</style>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('change', '.indikator-checkbox', function () {
            ...
            const indikatorLabel = $(this).closest('.form-check').find('.form-check-label').text().trim();
        ...
        if (currentLembagaId !== 'null') {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('.indikator-checkbox:checked').each(function() {
                        ...
                        let label = $(this).closest('.form-check').find('.form-check-label').text().trim();
                        let initialBobot = $(`#bobot-${id} input`).val() || ''; // Ambil bobot jika sudah ada
                ...
                error: function() {
                    $('#indikator-tree-view').html('<p class="text-danger text-center">Gagal memuat indikator. Silakan coba lagi.</p>');
                }
            });
            ...
            @endif
             ...
             $('#indikator-tree-view').html('<p class="text-danger text-center">Template belum memiliki Lembaga Akreditasi. Harap tentukan lembaga terlebih dahulu.</p>');
        }
