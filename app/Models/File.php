...
class File extends Model
{
    ...
    public function getTakeThumbnailAttribute() : string
    {
        $path = $this->target;
        $file_name = pathinfo($path, PATHINFO_FILENAME);
        $file_path_only = pathinfo($path, PATHINFO_DIRNAME);
        $file_extension = pathinfo($path, PATHINFO_EXTENSION);
        return Storage::disk($this->disk)->get($file_path_only.'/thumbnail-'.$file_name.'.'.$file_extension);
    }
    ...
    public function getLinkPublicStreamAttribute() : string
    {
        return url("/file-stream/{$this->kode_target}?id={$this->fileable_id}&code={$this->alias}");
    }
    public function getLinkPublicThumbnailStreamAttribute() : string
    {
        return url("/file-thumbnail/{$this->kode_target}?id={$this->fileable_id}&code={$this->alias}");
    }

    public function getKodeTargetAttribute()
    {
        return Str::of($this->target)->explode('/')->first();
    }
