...
class LogAktivitasAuditsController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $user = $request->user();
            $data=$this->model::with('hasilaudit','user')->get();
    ...
    {
		$data=[
			'hasil_audit_id'	=> \App\Models\HasilAudit::pluck('nama','id'),
			'user_id'	=> \App\Models\User::pluck('nama','id'),
		];
    ...
    {
        $request->validate([
					'tipe_aksi' => 'required|'.config('master.regex.json'),
					'catatan_aksi' => 'required|'.config('master.regex.json'),
					'hasil_audit_id' => 'required|'.config('master.regex.json'),
					'user_id' => 'required|'.config('master.regex.json'),
        ]);
