...
class TemplateKriteriasController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $user = $request->user();
            $data=$this->model::with('instrumentemplate','kriteria')->get();
    ...
    public function create()
    {
		$data=[
			...
			'kriteria_id'	=> \App\Models\Kriteria::pluck('nama','id'),
		];
    ...
    {
        $request->validate([
					'bobot' => 'required|'.config('master.regex.json'),
