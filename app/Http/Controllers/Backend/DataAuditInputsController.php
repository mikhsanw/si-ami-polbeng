...
class DataAuditInputsController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $user = $request->user();
            $data=$this->model::with('hasilaudit','indikator')->get();
    ...
    public function create()
    {
		$data=[
			...
			'indikator_input_id'	=> \App\Models\Indikator::pluck('nama','id'),
		];
    ...
    {
        $request->validate([
					'nilai_variable' => 'required|'.config('master.regex.json'),
					'hasil_audit_id' => 'required|'.config('master.regex.json'),
					'indikator_input_id' => 'required|'.config('master.regex.json'),
        ]);
