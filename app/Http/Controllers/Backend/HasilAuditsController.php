...
class HasilAuditsController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $user = $request->user();
            $data=$this->model::with('auditperiode','indikator')->get();
    ...
    {
        $request->validate([
					'skor_auditee' => 'required|'.config('master.regex.json'),
					'skor_final' => 'required|'.config('master.regex.json'),
					'catatan_final' => 'required|'.config('master.regex.json'),
					'status_terkini' => 'required|'.config('master.regex.json'),
					'audit_periode_id' => 'required|'.config('master.regex.json'),
					'indikator_id' => 'required|'.config('master.regex.json'),
        ]);
